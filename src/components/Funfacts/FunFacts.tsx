      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-8">
          {facts.map((fact, index) => (
            ...
            >
              <div className={`font-mono text-[10px] text-accent font-bold pb-2 mb-3 ${isMinimal ? 'border-b border-zinc-800/60' : 'border-b border-[#262626]'}`}>
                {isMinimal ? `INTEREST #${index + 1}` : `FACT_RECORD #${index + 1}`}
              </div>
              <p className={`text-sm text-white font-medium leading-snug ${isMinimal ? 'font-sans' : 'font-mono font-bold'}`}>
                "{fact.facts}"
              </p>
              <div className="mt-3 font-mono text-[9px] text-zinc-500">
                {isMinimal ? 'Verified Interest' : 'STATUS: VERIFIED'}
              </div>
            </div>
          ))}
        </div>
